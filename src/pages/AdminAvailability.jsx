...
export default function AdminAvailability() {
  ...
  const upcomingHolidays = holidays.filter((h) => h >= today).slice(0, 8);
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "0.4rem" }}>
          {upcomingHolidays.map((h) => (
            <div key={h} style={{
              ...
              <span style={{ color: "rgba(255,255,255,0.7)", fontSize: "0.82rem" }}>{formatDate(h)}</span>
            </div>
          ))}
        </div>
      </FHJCard>
