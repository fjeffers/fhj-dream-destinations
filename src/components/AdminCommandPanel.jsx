// src/components/AdminCommandPanel.jsx
import React, { useState } from "react";
import {
  FHJCard,
  FHJButton,
  FHJInput,
  fhjTheme,
} from "./fhj/FHJUIKit.jsx";

export default function AdminCommandPanel({ admin }) {
  const [cmd, setCmd] = useState("");
  const [response, setResponse] = useState(null);
  const [loading, setLoading] = useState(false);

  const run = async () => {
    if (!cmd.trim()) return;

    setLoading(true);

    const res = await fetch("/.netlify/functions/admin-command", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ command: cmd }),
    });

    const data = await res.json();
    setResponse(data);
    setLoading(false);
  };

  return (
    <FHJCard style={{ padding: "1.5rem", marginTop: "2rem" }}>
      <h3>FHJ Command Mode</h3>

      <FHJInput
        label="Enter command"
        value={cmd}
        onChange={(e) => setCmd(e.target.value)}
      />

      <FHJButton
        onClick={run}
        style={{ marginTop: "1rem", background: fhjTheme.primary }}
      >
        Run
      </FHJButton>

      {loading && <p style={{ marginTop: "1rem" }}>Processing...</p>}

      {response && (
        <div style={{ marginTop: "1.5rem" }}>
          <h4>{response.label}</h4>

          {response.type === "text" && (
            <p style={{ opacity: 0.7 }}>{response.result}</p>
          )}

          {response.type === "list" && (
            <ul style={{ paddingLeft: "1rem" }}>
              {response.result.map((item, i) => (
                <li key={i} style={{ marginBottom: "0.5rem" }}>
                  {Object.entries(item).map(([k, v]) => (
                    <p key={k}>
                      <strong>{k}:</strong> {v}
                    </p>
                  ))}
                </li>
              ))}
            </ul>
          )}

          {response.type === "timeline" && (
            <ul style={{ paddingLeft: "1rem" }}>
              {response.result.map((item, i) => (
                <li key={i} style={{ marginBottom: "1rem" }}>
                  <strong>{item.Admin}</strong> — {item.Action}
                  <p style={{ opacity: 0.7 }}>{item.Timestamp}</p>
                  <p>{item.Module}</p>
                </li>
              ))}
            </ul>
          )}
        </div>
      )}
    </FHJCard>
  );
}
