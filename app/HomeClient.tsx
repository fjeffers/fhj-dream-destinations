...
export default function HomeClient({ deals, heroContent = {}, footerContent = {} }: { deals: Deal[], heroContent?: any, footerContent?: any }) {
  ...
  // Hero defaults — overridden by Content Manager if set
  const hero = {
  ...
  return (
    <div style={{ background: C.cream, overflowX: 'hidden' }}>
      ...
      <section style={{ position: 'relative', height: '100vh', overflow: 'hidden' }}>
        ...
        {slides.map((s, i) => (
          <div key={i} style={{
            ...
            {/* Warm gradient overlay — NOT dark, golden warmth */}
            <div style={{
              position: 'absolute', inset: 0,
              background: 'linear-gradient(to right, rgba(46,35,24,0.72) 0%, rgba(46,35,24,0.38) 55%, rgba(46,35,24,0.1) 100%)'
            }} />
            <div style={{
              position: 'absolute', inset: 0,
              background: 'linear-gradient(to top, rgba(196,154,69,0.18) 0%, transparent 50%)'
            }} />
          </div>
        ))}
        ...
        <div style={{
          ...
          <div style={{ maxWidth: 780 }}>
            ...
            <div style={{
              opacity: loaded ? 1 : 0,
              animation: loaded ? 'fadeUp 0.9s ease forwards' : 'none',
              marginBottom: 24,
              marginTop: 160
            }}>
              <img src="/logo.png" alt="FHJ Dream Destinations"
                style={{ width: 180, height: 180, objectFit: 'contain', filter: 'drop-shadow(0 4px 24px rgba(196,154,69,0.5))' }} />
            </div>            {/* Eyebrow */}
            <div style={{
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 14, marginBottom: 18,
              opacity: loaded ? 1 : 0,
              animation: loaded ? 'fadeUp 0.9s ease 0.15s both' : 'none'
            }}>
              <div style={{ width: 36, height: 1.5, background: C.goldLight, borderRadius: 2 }} />
              <span style={{ fontFamily: 'Cinzel, serif', fontSize: 13, letterSpacing: 5, color: C.goldLight, fontWeight: 600 }}>
                {slides[slide].eyebrow.toUpperCase()}
              </span>
              <div style={{ width: 36, height: 1.5, background: C.goldLight, borderRadius: 2 }} />
            </div>
            ...
            <div style={{
              opacity: loaded ? 1 : 0,
              animation: loaded ? 'fadeUp 0.9s ease 0.25s both' : 'none'
            }}>
              <h1 style={{
                fontFamily: 'Cormorant Garamond, serif', fontWeight: 300,
                fontSize: 'clamp(54px, 8vw, 96px)', lineHeight: 0.97,
                color: '#FDF6EC', margin: 0,
                textShadow: '0 2px 24px rgba(46,35,24,0.3)'
              }}>
            ...
            <p style={{
              fontSize: 18, color: 'rgba(253,246,236,0.78)', lineHeight: 1.8,
              maxWidth: 520, margin: '28px auto 44px', fontWeight: 300,
              opacity: loaded ? 1 : 0,
              animation: loaded ? 'fadeUp 0.9s ease 0.38s both' : 'none'
            }}>
            ...
            <div style={{
              ...
              opacity: loaded ? 1 : 0,
              animation: loaded ? 'fadeUp 0.9s ease 0.48s both' : 'none'
            }}>
              <Link href="/book-appointment" className="slide-btn-primary" style={{
                background: C.gold, color: C.text,
                padding: '17px 48px', borderRadius: 6,
                fontFamily: 'Cinzel, serif', fontSize: 13, letterSpacing: 3, fontWeight: 800,
                textDecoration: 'none', display: 'inline-block',
                boxShadow: `0 10px 36px rgba(196,154,69,0.38)`,
                transition: 'all 0.3s'
              }}>PLAN MY VACATION</Link>
              <Link href="/about" className="slide-btn-ghost" style={{
                background: 'rgba(253,246,236,0.12)', color: '#FDF6EC',
                border: '1.5px solid rgba(232,200,122,0.45)',
                padding: '17px 40px', borderRadius: 6,
                fontFamily: 'Cinzel, serif', fontSize: 13, letterSpacing: 3,
                textDecoration: 'none', display: 'inline-block', transition: 'all 0.3s'
              }}>{hero.cta_secondary}</Link>
            </div>

            {/* Stats */}
            <div style={{
              display: 'flex', gap: 48, marginTop: 60, paddingTop: 36,
              borderTop: '1px solid rgba(232,200,122,0.25)',
              justifyContent: 'center',
              opacity: loaded ? 1 : 0,
              animation: loaded ? 'fadeUp 0.9s ease 0.58s both' : 'none'
            }}>
              {[['250+','Trips Crafted'],['48','Countries'],['98%','Return Rate']].map(([n, l]) => (
                <div key={l} style={{ textAlign: 'center' }}>
                  <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 38, color: C.goldLight, lineHeight: 1 }}>{n}</div>
                  <div style={{ fontSize: 13, color: 'rgba(253,246,236,0.55)', letterSpacing: 2, textTransform: 'uppercase', marginTop: 6, fontFamily: 'Cinzel, serif' }}>{l}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Slide dots — centered */}
        <div style={{ position: 'absolute', bottom: 36, left: '50%', transform: 'translateX(-50%)', zIndex: 10, display: 'flex', gap: 10, alignItems: 'center' }}>
          {slides.map((_, i) => (
            <button key={i} onClick={() => setSlide(i)} style={{
              width: slide === i ? 30 : 8, height: 8, borderRadius: 4,
              background: slide === i ? C.gold : 'rgba(232,200,122,0.4)',
      ...
      <section style={{ background: C.sand, borderBottom: `1px solid rgba(196,154,69,0.2)` }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '36px 60px', display: 'flex', justifyContent: 'space-around', alignItems: 'center' }}>
          {[
            ...
            <div key={i} style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
              ...
              <div style={{ fontFamily: 'Cinzel, serif', fontSize: 13, letterSpacing: 2, color: C.brown, fontWeight: 700 }}>{item.label.toUpperCase()}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
        {/* Left — warm cream with brand message */}
        <div style={{ background: C.sand, padding: '96px 72px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(34px,3.5vw,54px)', fontWeight: 300, color: C.text, lineHeight: 1.15, marginBottom: 24 }}>
            We don't just book trips.<br />
            <em style={{ color: C.teal }}>We craft experiences</em><br />
            you'll carry forever.
          </h2>
          <p style={{ fontSize: 18, color: C.muted, lineHeight: 1.9, marginBottom: 16 }}>
          ...
          <p style={{ fontSize: 18, color: C.muted, lineHeight: 1.9, marginBottom: 36 }}>
          ...
          <Link href="/about" style={{
            ...
            color: C.teal, fontFamily: 'Cinzel, serif', fontSize: 13,
            letterSpacing: 3, textDecoration: 'none', fontWeight: 700,
            borderBottom: `2px solid rgba(58,125,125,0.3)`, paddingBottom: 4, width: 'fit-content'
          }}>OUR STORY →</Link>
        </div>

        {/* Right — large stats on cream */}
        <div style={{ background: C.cream, padding: '96px 72px', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 0 }}>
          {[
            ...
            <div key={l} style={{
              ...
              padding: '28px 0',
              borderBottom: i < 3 ? `1px solid rgba(196,154,69,0.15)` : 'none'
            }}>
              <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 52, color: C.gold, lineHeight: 1, fontWeight: 300, minWidth: 96 }}>{n}</div>
              <div>
                <div style={{ fontFamily: 'Cinzel, serif', fontSize: 13, letterSpacing: 2, color: C.text, fontWeight: 700, marginBottom: 4 }}>{l.toUpperCase()}</div>
                <div style={{ fontSize: 18, color: C.muted, fontStyle: 'italic', fontFamily: 'Cormorant Garamond, serif' }}>{s}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 52 }}>
            <div>
              ...
              <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(36px,4vw,60px)', fontWeight: 300, color: C.text, lineHeight: 1 }}>
                Destinations & <em style={{ color: C.teal }}>Featured Deals</em>
              </h2>
            </div>
            <Link href="/book" style={{ fontFamily: 'Cinzel, serif', fontSize: 13, letterSpacing: 3, color: C.teal, textDecoration: 'none', fontWeight: 700, borderBottom: `1px solid rgba(58,125,125,0.3)`, paddingBottom: 2 }}>VIEW ALL →</Link>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16, marginBottom: 16 }}>
            {destinations.map((dest, i) => (
              <div key={dest.name} className="dest-card" style={{
                ...
                position: 'relative',
                height: i === 0 || i === 3 ? 340 : 260,
                boxShadow: `0 6px 28px rgba(196,154,69,0.12)`,
                ...
                <div className="dest-btn" style={{
                  position: 'absolute', inset: 0,
                  background: 'rgba(196,154,69,0.15)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  opacity: 0, transition: 'all 0.35s ease', transform: 'translateY(8px)'
                }}>
                  <Link href="/book-appointment" onClick={e => e.stopPropagation()} style={{
                    background: C.gold, color: C.text,
                    padding: '11px 30px', borderRadius: 6,
                    fontFamily: 'Cinzel, serif', fontSize: 13, letterSpacing: 2, fontWeight: 800,
                    textDecoration: 'none', boxShadow: '0 6px 20px rgba(196,154,69,0.4)'
                  }}>PLAN THIS TRIP</Link>
                </div>
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '18px 22px' }}>
                  <div style={{
                    display: 'inline-block', background: C.gold, borderRadius: 3,
                    padding: '3px 10px', fontFamily: 'Cinzel, serif', fontSize: 13,
                    letterSpacing: 2, color: C.text, fontWeight: 700, marginBottom: 6
                  }}>{dest.tag.toUpperCase()}</div>
                  <div style={{ fontFamily: 'Cinzel, serif', fontSize: 14, letterSpacing: 2, color: '#FDF6EC', fontWeight: 700 }}>{dest.name.toUpperCase()}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          {featured.length > 0 && (
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: 24, margin: '56px 0 48px' }}>
                <div style={{ flex: 1, height: 1, background: `rgba(196,154,69,0.2)` }} />
                <div style={{ fontFamily: 'Cinzel, serif', fontSize: 13, letterSpacing: 5, color: C.teal, fontWeight: 700, whiteSpace: 'nowrap' }}>
                  ✦ TEAM PICKS — FEATURED EXPERIENCES ✦
                </div>
                <div style={{ flex: 1, height: 1, background: `rgba(196,154,69,0.2)` }} />
              </div>

              {/* Deals — horizontal scroll feel, 3-col grid */}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 24 }}>
                {featured.map(deal => (
                  <div key={deal.id} className="deal-card" style={{
                    background: C.sand, borderRadius: 12, overflow: 'hidden',
                    border: `1px solid rgba(196,154,69,0.2)`,
                    boxShadow: `0 6px 28px rgba(196,154,69,0.1)`
                  }}>
                    <div style={{
                      height: 200, background: `linear-gradient(135deg, ${C.teal} 0%, #2d6666 100%)`,
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      fontSize: 76, position: 'relative'
                    }}>
                      {deal.image}
                      ...
                      fontSize: 13, letterSpacing: 2, color: C.text, fontWeight: 800
                    }}>{deal.category?.toUpperCase()}</div>
                  </div>
                  <div style={{ padding: '28px 28px 32px' }}>
                    <h3 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 27, color: C.text, marginBottom: 10, fontWeight: 400, lineHeight: 1.2 }}>{deal.title}</h3>
                    <p style={{ color: C.muted, fontSize: 18, lineHeight: 1.75, marginBottom: 22 }}>{deal.description}</p>
                    <div style={{ height: 1, background: `rgba(196,154,69,0.2)`, marginBottom: 22 }} />
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 22 }}>
                      <div>
                        <div style={{ fontFamily: 'Cinzel, serif', fontSize: 13, letterSpacing: 2, color: C.muted, marginBottom: 4 }}>FROM</div>
                        <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 36, color: C.teal, lineHeight: 1, fontWeight: 300 }}>{deal.price}</div>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontFamily: 'Cinzel, serif', fontSize: 13, letterSpacing: 2, color: C.muted, marginBottom: 4 }}>DURATION</div>
                        <div style={{ fontSize: 18, color: C.text, fontWeight: 700 }}>{deal.duration}</div>
                      </div>
                    </div>
                    <Link href="/book" style={{
                      display: 'block', textAlign: 'center', padding: '14px',
                      background: C.teal, color: '#FDF6EC',
                      fontFamily: 'Cinzel, serif', fontSize: 13, letterSpacing: 3,
                      textDecoration: 'none', borderRadius: 6, fontWeight: 700,
                      transition: 'background 0.3s'
                    }}>BOOK THIS EXPERIENCE</Link>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* ══════════════════════════════════
          MEET THE TEAM
      ══════════════════════════════════ */}
      <section style={{ padding: '100px 60px', background: C.tealLight }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 60 }}>
            ...
            <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(36px,4vw,58px)', fontWeight: 300, color: C.text, lineHeight: 1.1, marginBottom: 14 }}>
              Real people. Real <em style={{ color: C.teal }}>passion.</em>
            </h2>
            <p style={{ color: C.muted, fontSize: 18, maxWidth: 520, margin: '0 auto', lineHeight: 1.75 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
            {team.map((m, i) => (
              <div key={i} className="team-card" style={{
                background: C.cream, borderRadius: 12, padding: '44px 32px', textAlign: 'center',
                border: `1px solid rgba(196,154,69,0.2)`,
                boxShadow: `0 4px 20px rgba(196,154,69,0.08)`,
                transition: 'all 0.35s', position: 'relative', overflow: 'hidden'
              }}>
                <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 4, background: `linear-gradient(90deg, ${C.teal}, ${C.gold})` }} />
                <div style={{
                  width: 80, height: 80, borderRadius: '50%',
                  background: `linear-gradient(135deg, ${C.teal}, #2d6666)`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  margin: '0 auto 22px',
                  fontFamily: 'Cinzel, serif', fontSize: 22, color: '#FDF6EC', fontWeight: 700,
                  boxShadow: `0 8px 24px rgba(58,125,125,0.3)`
                }}>{m.initials}</div>
                <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 22, color: C.text, marginBottom: 6, fontWeight: 400 }}>{m.name}</div>
                <div style={{ fontFamily: 'Cinzel, serif', fontSize: 13, letterSpacing: 2, color: C.teal, marginBottom: 8, fontWeight: 700 }}>{m.role.toUpperCase()}</div>
                <div style={{ height: 1, background: `rgba(196,154,69,0.2)`, margin: '14px 0' }} />
                <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 18, color: C.muted, fontStyle: 'italic' }}>{m.specialty}</div>
              </div>
            ))}
          </div>
          <div style={{ textAlign: 'center', marginTop: 48 }}>
      ...
      {/* ══════════════════════════════════
          TESTIMONIALS
      ══════════════════════════════════ */}
      <section style={{ padding: '100px 60px', background: C.sand }}>
        <div style={{ maxWidth: 880, margin: '0 auto', textAlign: 'center' }}>
          ...
          <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(32px,4vw,54px)', fontWeight: 300, color: C.text, marginBottom: 56 }}>
            ...
            {testimonials.map((t, i) => (
              <div key={i} style={{
                ...
                <div style={{
                  ...
                  padding: '48px 56px',
                  border: `1px solid rgba(196,154,69,0.2)`,
                  boxShadow: `0 8px 40px rgba(196,154,69,0.1)`
                }}>
                  <div style={{ fontSize: 56, color: `rgba(196,154,69,0.35)`, fontFamily: 'Georgia', lineHeight: 1, marginBottom: 4 }}>"</div>
                  <p style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 21, color: C.text, lineHeight: 1.8, fontStyle: 'italic', marginBottom: 32 }}>{t.quote}</p>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16 }}>
                    <div style={{
                      width: 46, height: 46, borderRadius: '50%',
                      background: `linear-gradient(135deg, ${C.teal}, ${C.gold})`,
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      fontFamily: 'Cinzel, serif', fontSize: 13, color: '#FDF6EC', fontWeight: 700
                    }}>{t.initials}</div>
                    <div style={{ textAlign: 'left' }}>
                      <div style={{ fontFamily: 'Cinzel, serif', fontSize: 13, letterSpacing: 2, color: C.text, fontWeight: 700 }}>{t.name}</div>
                      <div style={{ fontSize: 18, color: C.muted, marginTop: 3 }}>{t.trip}</div>
                    </div>
                    <div style={{ display: 'flex', gap: 3, marginLeft: 8 }}>
                      {[...Array(5)].map((_, s) => <span key={s} style={{ fontSize: 18, color: C.gold }}>★</span>)}
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginTop: 296 }}>
            {testimonials.map((_, i) => (
              <button key={i} onClick={() => setActiveT(i)} style={{
                width: activeT === i ? 28 : 8, height: 8, borderRadius: 4,
                background: activeT === i ? C.teal : `rgba(58,125,125,0.25)`,
                border: 'none', cursor: 'pointer', transition: 'all 0.3s', padding: 0
              }} />
            ))}
          </div>
      ...
      <section style={{ padding: '100px 60px', background: C.cream }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 60 }}>
            ...
            <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(36px,4vw,58px)', fontWeight: 300, color: C.text }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16 }}>
            {[
              ...
              <div key={i} className="why-card" style={{
                background: C.sand, borderRadius: 12, padding: '36px 32px',
                display: 'flex', gap: 24, alignItems: 'flex-start',
                border: `1px solid rgba(196,154,69,0.18)`,
                boxShadow: `0 4px 20px rgba(196,154,69,0.07)`,
                transition: 'all 0.4s', cursor: 'default'
              }}>
                <div className="why-icon" style={{
                  fontSize: 36, flexShrink: 0,
                  width: 64, height: 64, borderRadius: 12,
                  background: `rgba(58,125,125,0.1)`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  transition: 'all 0.4s'
                }}>{item.icon}</div>
                <div>
                  <div className="why-title" style={{ fontFamily: 'Cinzel, serif', fontSize: 13, letterSpacing: 2, color: C.text, fontWeight: 700, marginBottom: 12, transition: 'color 0.4s' }}>{item.title.toUpperCase()}</div>
                  <p className="why-body" style={{ fontSize: 18, color: C.muted, lineHeight: 1.85, margin: 0, transition: 'color 0.4s' }}>{item.body}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ══════════════════════════════════
          CTA — Full photo with warm overlay
      ══════════════════════════════════ */}
      <section style={{ position: 'relative', height: 540, overflow: 'hidden' }}>
        <img src="https://images.unsplash.com/photo-1548574505-5e239809ee19?auto=format&fit=crop&w=1800&q=80"
          alt="Luxury Travel" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
        {/* Warm NOT dark overlay */}
        ...
        <div style={{
          ...
          <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(42px,5.5vw,76px)', color: '#FDF6EC', fontWeight: 300, lineHeight: 1.05, marginBottom: 20, maxWidth: 640 }}>
            {hero.headline1}<br />
            <em style={{ color: C.goldLight }}>{hero.headline2}</em>
          </h2>
          <p style={{ color: 'rgba(253,246,236,0.75)', fontSize: 18, lineHeight: 1.8, marginBottom: 44, maxWidth: 480 }}>
            {hero.subtext}
          </p>
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            <Link href="/book-appointment" className="slide-btn-primary" style={{
              ...
              fontFamily: 'Cinzel, serif', fontSize: 13, letterSpacing: 3, fontWeight: 800,
              textDecoration: 'none', display: 'inline-block',
              boxShadow: `0 10px 36px rgba(196,154,69,0.4)`,
              transition: 'all 0.3s'
            }}>{hero.cta_primary}</Link>
            <Link href="/about" className="slide-btn-ghost" style={{
              background: 'rgba(253,246,236,0.12)', color: '#FDF6EC',
              border: '1.5px solid rgba(232,200,122,0.4)',
              padding: '18px 40px', borderRadius: 6,
