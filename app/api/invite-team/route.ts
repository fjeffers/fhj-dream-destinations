...
export async function POST(req: Request) {
  try {
    ...
    if (profileError) {
      ...
      // Don't block — user was created in auth, profile can be fixed manually
    }

    // Send email — non-fatal: if Resend key is missing, user still gets created
    if (process.env.RESEND_API_KEY) {
      try {
            ...
            <div style="font-family: system-ui, sans-serif; background: #FDFAF3; padding: 0;">
              <div style="max-width: 560px; margin: auto; background: white; border: 1px solid rgba(196,154,10,0.2); overflow: hidden;">
                ...
                <div style="padding: 40px;">
                  ...
                  <p style="font-size: 15px; color: #5a4a3a; line-height: 1.7;">You've been added to the FHJ Dream Destinations admin team as a <strong>${role}</strong>. Use the credentials below to log in:</p>
                  ...
                  <p style="font-size: 14px; color: #8a7a6a; line-height: 1.6;">⚠ Please change your password after your first login via your profile settings.</p>
                  <p style="font-size: 14px; color: #5a4a3a; margin-top: 24px;">Questions? Reply to this email or reach us at <a href="mailto:info@fhjdreamdestinations.com" style="color: #076060;">info@fhjdreamdestinations.com</a>.</p>
        ...
        // Log but don't fail — user account was created successfully
        console.error('Email send failed (non-fatal):', emailError)
        // Return success with a warning so the UI can inform the admin
        return NextResponse.json({ success: true, emailWarning: 'User created but invite email could not be sent. Please share login details manually.' })
      }
    } else {
      console.warn('RESEND_API_KEY not set — skipping invite email.')
      return NextResponse.json({ success: true, emailWarning: `RESEND_API_KEY not configured. User created. Share credentials manually: ${email} / ${tempPassword}` })
    }
