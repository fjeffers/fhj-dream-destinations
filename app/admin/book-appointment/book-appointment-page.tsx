    ...
    <div style={{ minHeight: '100vh', background: 'var(--ivory)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ maxWidth: 520, textAlign: 'center', animation: 'fadeUp 0.8s ease' }}>
        ...
        <div style={{ background: 'white', border: '1px solid rgba(196,154,10,0.25)', borderRadius: 8, padding: '16px 24px', marginBottom: 32 }}>
          <div style={{ fontFamily: 'Cinzel, serif', fontSize: 9, letterSpacing: 2, color: 'var(--teal-dark)', marginBottom: 6 }}>CONFIRMATION ID</div>
          <div style={{ fontFamily: 'monospace', fontSize: 14, color: 'var(--text-rich)', wordBreak: 'break-all' }}>{confirmId}</div>
        </div>
    ...
    <div style={{ minHeight: '100vh', background: 'var(--ivory)' }}>
      <div style={{ background: 'white', borderBottom: '2px solid rgba(196,154,10,0.2)', padding: '20px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Link href="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            <div style={{ fontFamily: 'Cinzel, serif', fontSize: 13, color: 'var(--gold-dark)', fontWeight: 700, letterSpacing: 2 }}>FHJ DREAM</div>
            <div style={{ fontFamily: 'Cinzel, serif', fontSize: 8, letterSpacing: 4, color: 'var(--teal-dark)', fontWeight: 600 }}>DESTINATIONS</div>
          </div>
        </Link>
        <div style={{ fontFamily: 'Cinzel, serif', fontSize: 10, letterSpacing: 3, color: 'var(--teal-dark)', fontWeight: 600 }}>SCHEDULE A CONSULTATION</div>
      </div>

      <div style={{ maxWidth: 900, margin: '0 auto', padding: '60px 24px' }}>
        <div style={{ textAlign: 'center', marginBottom: 52 }}>
          <div style={{ fontFamily: 'Cinzel, serif', fontSize: 11, letterSpacing: 6, color: 'var(--teal)', marginBottom: 14, fontWeight: 600 }}>BOOK AN APPOINTMENT</div>
          ...
          {['Select Date', 'Choose Time', 'Your Details'].map((s, i) => (
            <div key={s} style={{ display: 'flex', alignItems: 'center' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontFamily: 'Cinzel, serif', fontSize: 9, letterSpacing: 2, color: step === i + 1 ? 'var(--teal-dark)' : 'var(--muted)', fontWeight: 600 }}>{s}</span>
              </div>
        ...
        {step === 1 && (
          <div style={{ background: 'white', borderRadius: 12, border: '1px solid rgba(196,154,10,0.2)', boxShadow: '0 4px 32px rgba(0,0,0,0.06)', padding: 36 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
              ...
              <span style={{ fontFamily: 'Cinzel, serif', fontSize: 14, letterSpacing: 2, color: 'var(--text-rich)', fontWeight: 700 }}>{MONTHS[month]} {year}</span>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4, marginBottom: 8 }}>
              {DAYS.map(d => <div key={d} style={{ textAlign: 'center', fontFamily: 'Cinzel, serif', fontSize: 10, letterSpacing: 2, color: 'var(--teal-dark)', padding: '6px 0', fontWeight: 700 }}>{d}</div>)}
            </div>
            ...
            <div style={{ display: 'flex', gap: 20, marginTop: 20, paddingTop: 16, borderTop: '1px solid rgba(196,154,10,0.15)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--muted)' }}><div style={{ width: 14, height: 14, borderRadius: 3, background: 'rgba(14,143,143,0.06)', border: '1px solid rgba(14,143,143,0.2)' }} /> Available</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--muted)' }}><div style={{ width: 14, height: 14, borderRadius: 3, background: 'var(--teal-dark)' }} /> Selected</div>
            </div>
        ...
        {step === 2 && selectedDate && (
          <div style={{ background: 'white', borderRadius: 12, border: '1px solid rgba(196,154,10,0.2)', boxShadow: '0 4px 32px rgba(0,0,0,0.06)', padding: 36 }}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: 15, marginBottom: 28 }}>Select a 30-minute time slot</p>
        ...
        {step === 3 && (
          <div style={{ background: 'white', borderRadius: 12, border: '1px solid rgba(196,154,10,0.2)', boxShadow: '0 4px 32px rgba(0,0,0,0.06)', padding: 36 }}>
            <div style={{ background: 'rgba(14,143,143,0.06)', border: '1px solid rgba(14,143,143,0.2)', borderRadius: 8, padding: '16px 20px', marginBottom: 32, display: 'flex', gap: 24 }}>
              <div><div style={{ fontFamily: 'Cinzel, serif', fontSize: 9, letterSpacing: 2, color: 'var(--teal-dark)', marginBottom: 4 }}>DATE</div><div style={{ fontSize: 15, color: 'var(--text-rich)', fontWeight: 600 }}>{selectedDate?.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</div></div>
              <div style={{ width: 1, background: 'rgba(14,143,143,0.2)' }} />
              <div><div style={{ fontFamily: 'Cinzel, serif', fontSize: 9, letterSpacing: 2, color: 'var(--teal-dark)', marginBottom: 4 }}>TIME</div><div style={{ fontSize: 15, color: 'var(--text-rich)', fontWeight: 600 }}>{selectedTime}</div></div>
            </div>
