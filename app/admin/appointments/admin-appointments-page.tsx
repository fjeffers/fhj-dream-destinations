    ...
    try {
      ...
      await emailjs.send(
        ...
        'template_0ai8is3',
        {
          ...
          to_name: clientName || 'Valued Client',
      ...
      alert(`Failed to send: ${err.message}. Try copying the link instead.`)
      setSending(false)
    }
  }
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 32 }}>
        ...
        <div style={{ background: 'white', border: '2px solid rgba(196,154,10,0.25)', borderRadius: 8, padding: 24, boxShadow: '0 2px 16px rgba(196,154,10,0.08)' }}>
          <div style={{ fontFamily: 'Cinzel, serif', fontSize: 10, letterSpacing: 2, color: 'var(--teal-dark)', marginBottom: 10, fontWeight: 700 }}>🌐 PUBLIC BOOKING LINK</div>
          <div style={{ fontSize: 13, color: 'var(--text-rich)', fontFamily: 'monospace', background: 'var(--ivory)', padding: '10px 14px', borderRadius: 4, border: '1px solid rgba(196,154,10,0.2)', marginBottom: 12, wordBreak: 'break-all' }}>
            {baseUrl}/book-appointment
          </div>
          <p style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 14 }}>Anyone with this link can book a consultation.</p>
        ...
        <div style={{ background: 'linear-gradient(135deg, var(--teal-dark), var(--teal))', borderRadius: 8, padding: 24, boxShadow: '0 4px 24px rgba(14,143,143,0.25)' }}>
          <div style={{ fontFamily: 'Cinzel, serif', fontSize: 10, letterSpacing: 2, color: 'rgba(255,255,255,0.8)', marginBottom: 10, fontWeight: 700 }}>✦ SEND TO A CLIENT</div>
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.85)', lineHeight: 1.65, marginBottom: 18 }}>
              ...
              {requests.map(r => (
                <tr key={r.id}>
                  <td>
                    ...
                    {r.notes && <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2, maxWidth: 180 }}>{r.notes.slice(0, 50)}{r.notes.length > 50 ? '…' : ''}</div>}
                  </td>
                  <td>
                    <div style={{ fontWeight: 600 }}>{new Date(r.date + 'T12:00:00').toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}</div>
                    <div style={{ color: 'var(--teal-dark)', fontSize: 13, marginTop: 2 }}>{r.time}</div>
                  </td>
                  <td><span className="badge badge-teal">{r.type}</span></td>
                  <td>
                    <div style={{ fontSize: 14 }}>{r.email}</div>
                    {r.phone && <div style={{ fontSize: 13, color: 'var(--muted)' }}>{r.phone}</div>}
                  </td>
                  <td>
                    ...
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                      {r.status !== 'Confirmed' && (
                        <button onClick={() => updateStatus(r.id, 'Confirmed')}
                          style={{ background: 'rgba(26,122,74,0.1)', border: '1px solid rgba(26,122,74,0.3)', color: 'var(--success)', borderRadius: 4, padding: '6px 10px', cursor: 'pointer', fontSize: 11, fontFamily: 'Cinzel, serif', letterSpacing: 1, fontWeight: 600 }}>
                          Confirm
                        </button>
                      )}
                      {r.status !== 'Cancelled' && (
                        <button onClick={() => updateStatus(r.id, 'Cancelled')}
                          style={{ background: 'rgba(192,57,43,0.08)', border: '1px solid rgba(192,57,43,0.25)', color: 'var(--danger)', borderRadius: 4, padding: '6px 10px', cursor: 'pointer', fontSize: 11, fontFamily: 'Cinzel, serif', letterSpacing: 1, fontWeight: 600 }}>
                          Cancel
                        </button>
                      )}
                    </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          ...
          <div style={{ background: 'white', borderRadius: 12, width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 24px 80px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ padding: '24px 28px', borderBottom: '1px solid rgba(196,154,10,0.15)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontFamily: 'Cinzel, serif', fontSize: 10, letterSpacing: 3, color: 'var(--teal)', marginBottom: 4, fontWeight: 700 }}>SEND BOOKING LINK</div>
            ...
            <div style={{ padding: 28 }}>
              ...
              {sendMethod === 'copy' && (
                <div style={{ textAlign: 'center' }}>
                  ...
                  <p style={{ color: 'var(--muted)', fontSize: 13, marginTop: 12 }}>Paste into any message, email, or text</p>
                </div>
              )}
              ...
              {sendMethod === 'email' && (
                <div>
                  ...
                  {sent ? (
                    ...
                    <button className="btn-teal" style={{ borderRadius: 4, padding: '13px 48px', fontSize: 12, opacity: sending || !clientEmail ? 0.6 : 1 }}
              ...
              {sendMethod === 'sms' && (
                <div>
                  ...
                  <a href={`sms:${clientPhone}?body=${encodeURIComponent(`Hi ${clientName || 'there'}! Here's your personal booking link to schedule a consultation with FHJ Dream Destinations: ${shareLink}`)}`}
                    className="btn-teal" style={{ borderRadius: 4, padding: '13px 48px', fontSize: 12, display: 'inline-block', opacity: clientPhone ? 1 : 0.5, pointerEvents: clientPhone ? 'auto' : 'none' }}>
                    💬 Open in Messages
                  </a>
                  <p style={{ color: 'var(--muted)', fontSize: 12, marginTop: 10 }}>Opens your default SMS app with the link pre-filled.</p>
                </div>
              )}
              ...
              {sendMethod === 'qr' && (
                <div style={{ textAlign: 'center' }}>
                  ...
                  <p style={{ color: 'var(--muted)', fontSize: 12, marginTop: 12 }}>Client scans the QR code to open booking page directly</p>
                </div>
              )}
