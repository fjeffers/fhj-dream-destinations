...
export default async function AboutPage() {
  ...
  // Load editable content — fall back to defaults if not in DB yet
  const { data: rows } = await supabase
    .from('site_content')
    .select('section, content')
    .in('section', ['about_story', 'about_mission', 'about_values', 'about_milestones'])

  const get = (key: string, def: any) => {
    const row = rows?.find(r => r.section === key)
    if (!row) return def
    return Array.isArray(def) ? (Array.isArray(row.content) ? row.content : def) : { ...def, ...row.content }
  }

  const story = get('about_story', DEFAULT_STORY)
  const mission = get('about_mission', DEFAULT_MISSION)
  const values = get('about_values', DEFAULT_VALUES)
  const milestones = get('about_milestones', DEFAULT_MILESTONES)
  ...
  return (
    <div style={{ background: '#FDFAF3', overflowX: 'hidden' }}>
      ...
      {/* ── HERO ── */}
      <div className="about-hero" style={{ minHeight: '88vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '140px 40px 100px', background: 'linear-gradient(160deg, #FDFAF3 0%, #EDF7F7 55%, #FDFAF3 100%)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 820 }}>
          <div className="about-fade" style={{ fontFamily: 'Cinzel, serif', fontSize: 11, letterSpacing: 7, color: '#3A7D7D', marginBottom: 20, fontWeight: 600, animationDelay: '0.1s' }}>✦ OUR STORY ✦</div>
      ...
      <section style={{ background: '#F5ECD7', borderTop: '1px solid rgba(196,154,10,0.15)', borderBottom: '1px solid rgba(196,154,10,0.15)' }}>
        <div className="section-inner origin-grid" style={{ maxWidth: 1100, margin: '0 auto', padding: '100px 60px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }}>
          <div>
            ...
            <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(32px, 4vw, 52px)', fontWeight: 300, color: '#2E2318', lineHeight: 1.1, marginBottom: 28 }}>A Passion That<br /><em style={{ color: '#3A7D7D' }}>Became a Purpose</em></h2>
            <div style={{ width: 56, height: 3, background: 'linear-gradient(90deg, #3A7D7D, #C49A45)', marginBottom: 28, borderRadius: 2 }} />
            <p style={{ fontSize: 17, color: '#8A7A6A', lineHeight: 1.95, marginBottom: 22 }}>{story.p1}</p>
            <p style={{ fontSize: 17, color: '#8A7A6A', lineHeight: 1.95, marginBottom: 22 }}>{story.p2}</p>
            <p style={{ fontSize: 17, color: '#8A7A6A', lineHeight: 1.95 }}>{story.p3}</p>
          </div>
          <div style={{ position: 'relative' }}>
            ...
            <div style={{ background: 'white', border: '1px solid rgba(196,154,10,0.2)', borderRadius: 12, padding: '48px 40px', boxShadow: '0 12px 48px rgba(14,143,143,0.09)', position: 'relative', overflow: 'hidden' }}>
              ...
              <p style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 21, fontStyle: 'italic', color: '#2E2318', lineHeight: 1.8, marginBottom: 32 }}>"{story.founder_quote}"</p>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'linear-gradient(135deg, #076060, #3A7D7D)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontFamily: 'Cinzel, serif', fontSize: 16, fontWeight: 700, flexShrink: 0 }}>
                  {story.founder_name?.split(' ').map((n: string) => n[0]).join('').slice(0,2) || 'HJ'}
                </div>
                <div>
                  <div style={{ fontFamily: 'Cinzel, serif', fontSize: 12, color: '#2E2318', fontWeight: 700, letterSpacing: 1, marginBottom: 4 }}>{story.founder_name}</div>
                  <div style={{ fontSize: 14, color: '#3A7D7D', fontStyle: 'italic', fontFamily: 'Cormorant Garamond, serif' }}>{story.founder_title}</div>
      ...
      <div style={{ background: '#3A7D7D', padding: '60px 40px' }}>
        <div className="milestones-grid" style={{ maxWidth: 1000, margin: '0 auto', display: 'grid', gridTemplateColumns: `repeat(${milestones.length}, 1fr)`, gap: 40, textAlign: 'center' }}>
      ...
      <section className="section-inner" style={{ maxWidth: 820, margin: '0 auto', padding: '100px 60px', textAlign: 'center' }}>
        ...
        <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(32px, 4.5vw, 56px)', fontWeight: 300, color: '#2E2318', lineHeight: 1.1, marginBottom: 28 }}>
          <em style={{ color: '#3A7D7D' }}>{mission.heading}</em>
        </h2>
        ...
        <p style={{ fontSize: 18, color: '#8A7A6A', lineHeight: 1.95, marginBottom: 20 }}>{mission.p1}</p>
        <p style={{ fontSize: 18, color: '#8A7A6A', lineHeight: 1.95 }}>{mission.p2}</p>
      </section>
      ...
      <section style={{ background: '#F5ECD7', borderTop: '1px solid rgba(196,154,10,0.15)', borderBottom: '1px solid rgba(196,154,10,0.15)', padding: '90px 40px' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 60 }}>
            ...
            <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(32px, 4vw, 52px)', fontWeight: 300, color: '#2E2318' }}>What Sets Us <em style={{ color: '#3A7D7D' }}>Apart</em></h2>
          </div>
      ...
      <div className="cta-inner" style={{ background: 'linear-gradient(135deg, #076060 0%, #3A7D7D 60%, #076060 100%)', padding: '100px 60px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 640, margin: '0 auto' }}>
          ...
          <div className="cta-btns" style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href="/book-appointment" style={{ background: '#C49A45', color: '#2E2318', padding: '16px 52px', fontFamily: 'Cinzel, serif', fontSize: 12, letterSpacing: 3, textDecoration: 'none', display: 'inline-block', borderRadius: 4, fontWeight: 800, boxShadow: '0 8px 32px rgba(196,154,10,0.4)' }}>BOOK FREE CONSULTATION</Link>
            <Link href="/book" style={{ background: 'rgba(255,255,255,0.1)', color: 'white', border: '1.5px solid rgba(255,255,255,0.4)', padding: '16px 40px', fontFamily: 'Cinzel, serif', fontSize: 11, letterSpacing: 3, textDecoration: 'none', display: 'inline-block', borderRadius: 4 }}>SUBMIT AN INQUIRY</Link>
          </div>
        </div>
      </div>
