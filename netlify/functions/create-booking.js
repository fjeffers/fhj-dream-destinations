...
const { supabase, respond, selectRecords, submitToAirtable } = require('./utils');
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const formula = `LOWER({Email}) = '${String(email).toLowerCase()}'`;
    const clientMatches = await selectRecords("Client Name", formula);
